...
async function checkAdmissionPayment(transactionId: string) {
  const url = new URL(
...
export async function POST(request: NextRequest) {
  ...
  try {
    const results = await Promise.all(ids.map((id) => checkAdmissionPayment(id)));
    ...
    return NextResponse.json(
      {
        message:
          results.length === 1
            ? results[0]?.message || "Admission payment lookup completed."
            : `${successCount}/${results.length} admission payment lookup${
                results.length === 1 ? "" : "s"
              } successful.`,
        results,
        success
